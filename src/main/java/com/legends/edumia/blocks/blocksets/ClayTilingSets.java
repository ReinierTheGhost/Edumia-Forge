...
public class ClayTilingSets {
    ...
    public record ClayTilingSet(RegistryObject<Block> block, RegistryObject<Block> slab, RegistryObject<Block> stairs, RegistryObject<Block> corner) {
    }

    public static ClayTilingSet registerClaySet(String name, float strength, float resistance) {

        RegistryObject<Block> block = registerBlock(name + "clay_tiling",() -> new Block(BlockBehaviour.Properties.copy(Blocks.STONE)
                .strength(strength, resistance).sound(SoundType.STONE).requiresCorrectToolForDrops()));


        RegistryObject<Block> slab = registerBlock(name + "clay_tiling_slab", () -> new AxialSlabBlock(block));

        RegistryObject<Block> stairs = registerBlock(name + "clay_tiling_stairs", () -> new StairBlock(block.get().defaultBlockState(),
                BlockBehaviour.Properties.copy(block.get()).strength(strength, resistance).sound(SoundType.STONE).requiresCorrectToolForDrops()));

        RegistryObject<Block> corner = registerBlock(name +"clay_tiling_vertical_corner", () -> new VerticalCorner(BlockBehaviour.Properties
                .copy(block.get()).strength(strength, resistance).requiresCorrectToolForDrops()));
