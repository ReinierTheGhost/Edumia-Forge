...
public class TextureConnectionProperties {
    ...
    public Optional<ResourceLocation> getElementIconPath(ConnectedTextureElement element){
        if (element == ConnectedTextureElement.BASE){
            ...
            return this.elementPathOverrides.isPresent() ? Optional.ofNullable((this.elementPathOverrides.get()).get(element)) :
                    Optional.of(new ResourceLocation(this.textureName + element.getDefaultIconSuffix()));
        }
    }
    ...
    public ResourceLocation getCanonicalCacheKey() {
        return new ResourceLocation(this.textureName.getNamespace(), String.format("%s.connectedpropertiesoverrides_%sincludebaseelement%s__makefromsingleicon%s", this.textureName.getPath(), this.getCanonicalFormForElementPathOverrides(), this.includeBaseElement, this.makeFromSingleIcon));
    }
